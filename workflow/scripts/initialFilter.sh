...
plink2 --pfile $INPUT --mind $INITIAL_SUBJECT_MISSINGNESS --make-pgen --out ${INTER_FILEPREFIX}_2  --threads $THREADS --rm-dup exclude-all \

# Marker missingness final filter 
plink2 --pfile ${INTER_FILEPREFIX}_2 --geno $FINAL_VARIANT_MISSINGNESS --make-pgen --out ${INTER_FILEPREFIX}_3  --threads $THREADS

# Sample missingness final filter
plink2 --pfile ${INTER_FILEPREFIX}_3 --mind $FINAL_SUBJECT_MISSINGNESS --make-pgen --out $OUTPRE --threads $THREADS
