...
def active_adjhe_config():
    if run_by_ancestry():
        return bool(
            ...
            and SNP_HERIT_CONFIG.get("source_grm_prefix")
            and SNP_HERIT_CONFIG.get("source_bfile_prefix")
        )
    return bool(
        SNP_HERIT_CONFIG.get("pheno")
        and SNP_HERIT_CONFIG.get("covar")
        and SNP_HERIT_CONFIG.get("grm_prefix")
    )
...
if SNP_HERIT_ACTIVE and run_by_ancestry():
    ...
    def _grm_prefix():
        return str(SNP_HERIT_CONFIG.get("grm_prefix", ""))


    def _bed_prefix():
        return str(SNP_HERIT_CONFIG.get("bfile_prefix", _grm_prefix()))
    ...
    rule prepareABCDHeritInputs:
        ...
        params:
            ...
            match_on=SNP_HERIT_CONFIG.get("match_on", "IID"),
        ...
        run:
            ...
            grm_id_path = Path(f"{params.grm_prefix}.grm.id")
            ...
            if drop_missing_rows:
                keep_cols = mpheno_names + all_covar_cols
            ...
            if merged.empty:
                raise ValueError(
                    "No samples remain after intersecting GRM IDs with phenotype/covariate data."
                )
                ...
                json.dump(
                    {
                        ...
                        "match_on": match_on,
                        "force_fid_from_iid": force_fid_from_iid,
                        "n_grm": int(len(grm_ids)),
                        "n_merged_before_dropna": int(before_drop),
    ...
    rule prepareAdjHEAnalysisSet:
        ...
        resources:
            ...
            runtime=1440,
        input:
            keep=rules.prepareABCDHeritInputs.output.keep,
            bed=lambda wildcards: f"{_bed_prefix()}.bed",
            bim=lambda wildcards: f"{_bed_prefix()}.bim",
            fam=lambda wildcards: f"{_bed_prefix()}.fam",
        output:
            bed=OUT_DIR
            / "{subset}"
            / "03-snpHeritability"
    ...
    rule estimateSnpHeritabilityAdjHE:
        threads: 1
        resources:
            nodes=1,
            mem_mb=32000,
            runtime=2880,
        input:
            pheno=rules.prepareABCDHeritInputs.output.pheno_phen,
            covar=rules.prepareABCDHeritInputs.output.covar_txt,
            manifest=rules.prepareABCDHeritInputs.output.manifest,
            grm=rules.prepareAdjHEAnalysisSet.output.grm,
            grmid=rules.prepareAdjHEAnalysisSet.output.grmid,
            grmN=rules.prepareAdjHEAnalysisSet.output.grmN,
            pca=rules.prepareAdjHEAnalysisSet.output.pca_adjhe,
        output:
...
if SNP_HERIT_ACTIVE and run_by_ancestry():
    ...
    rule prepareABCDHeritInputsByAncestry:
        ...
        output:
            pheno_tsv=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_input.pheno.tsv",
            covar_tsv=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_input.covar.tsv",
            pheno_phen=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_input.pheno.phen",
            covar_txt=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_input.covar.txt",
            keep=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_input.keep",
            manifest=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_input_manifest.json",
        params:
            pheno_files=json.dumps(ensure_list(SNP_HERIT_CONFIG.get("pheno"))),
            covar_files=json.dumps(ensure_list(SNP_HERIT_CONFIG.get("covar"))),
            mpheno_names=json.dumps(ensure_list(SNP_HERIT_CONFIG.get("mpheno"))),
            qcovar=json.dumps(ensure_list(SNP_HERIT_CONFIG.get("qcovar"))),
            covar_discrete=json.dumps(ensure_list(SNP_HERIT_CONFIG.get("covar_discrete"))),
            iid_col=SNP_HERIT_CONFIG.get("iid_col", "IID"),
            fid_col=SNP_HERIT_CONFIG.get("fid_col", "FID"),
            pheno_filter=json.dumps(SNP_HERIT_CONFIG.get("pheno_filter")),
            covar_filter=json.dumps(SNP_HERIT_CONFIG.get("covar_filter")),
        ...
        run:
            ...
            def merge_tables(paths, query_string):
                ...
                for df in dfs[1:]:
                    ...
                    if "FID_x" in merged.columns:
                        merged["FID"] = merged["FID_x"]
                        drop_cols = [c for c in ["FID_x", "FID_y"] if c in merged.columns]
                        merged = merged.drop(columns=drop_cols)
            ...
            pheno_out.to_csv(output.pheno_phen, sep=" ", index=False, header=False)
            covar_out.to_csv(output.covar_txt, sep=" ", index=False, header=False)
            merged[["FID", "IID"]].to_csv(output.keep, sep=" ", index=False, header=False)
    ...
    rule prepareAdjHEAnalysisSetByAncestry:
        ...
        output:
            bed=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_analysis" / "unrelated_shared.bed",
            bim=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_analysis" / "unrelated_shared.bim",
            fam=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_analysis" / "unrelated_shared.fam",
            grm=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_analysis" / "unrelated_shared.grm.bin",
            grmid=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_analysis" / "unrelated_shared.grm.id",
            grmN=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_analysis" / "unrelated_shared.grm.N.bin",
            pca_raw=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_analysis" / "unrelated_shared.eigenvec",
            pca_adjhe=OUT_DIR / "{subset}" / "03-snpHeritability" / "adjhe_analysis" / "unrelated_shared_adjhe.eigenvec",
        params:
            plink2_bin=_plink2_bin(),
            npc=SNP_HERIT_CONFIG.get("npc", 10),
