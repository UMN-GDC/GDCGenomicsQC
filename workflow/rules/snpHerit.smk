...
if SIM_CFG.get("enabled", False):
    ...
    if len(sim_ancestries) == 2:
        SIM_OUT_DIR = OUT_DIR / "simulations" / f"{sim_ancestries[0]}_{sim_ancestries[1]}"
        ...
        rule estimateSnpHeritabilitySimulated:
            ...
            input:
                ...
                grm_Nbin=SIM_OUT_DIR / "{anc}_simulation.grm.N.bin",
                eigenvec=SIM_OUT_DIR / "{anc}_simulation.eigenvec",
                pheno=SIM_OUT_DIR / "{anc}_simulation_pheno1.pheno",
            output:
                estimates=SIM_OUT_DIR / "{anc}_herit.csv",
            params:
                grm_prefix=SIM_OUT_DIR / "{anc}_simulation",
                out_prefix=SIM_OUT_DIR / "{anc}_herit",
            run:
                import json
                import os
