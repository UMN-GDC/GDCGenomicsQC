...
plink --bfile ${INTER_FILEPREFIX}_7 --make-bed --out $STAGE/standardFilter  --threads $THREADS

# Extract snps retained in $OUTDIR.QC8
plink --bfile $STAGE/standardFilter --write-snplist --out $STAGE/standardFilter  --threads $THREADS

plink --bfile $STAGE/standardFilter --indep-pairwise 500 10 0.1 --out $STAGE/standardFilter  --threads $THREADS
plink --bfile $STAGE/standardFilter --extract $STAGE/standardFilter.prune.in --make-bed --out $STAGE/standardFilter.LDpruned  --threads $THREADS
