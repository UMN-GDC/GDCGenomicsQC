...
cls <- c("character", "character", "numeric", "integer", "character", "character")
ref <- read.table(ref_path, header = FALSE, sep = "\t", colClasses = cls)
tgt <- read.table(tgt_path, header = FALSE, sep = "\t", colClasses = cls)

names(ref) <- names(tgt) <- c("chr", "id", "cm", "pos", "ref", "alt")
...
matched_pos   <- matched_exact
matched_pos   <- sum(!is.na(tgt$rsid) & tgt$rsid != tgt$id) - matched_exact
unmatched     <- sum(tgt$rsid == tgt$id & is.na(exact_map[tgt$exact_key]))
