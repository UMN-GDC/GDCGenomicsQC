...
for(mmm in 1:M){
  ...
  registerDoMC(1)
  param <- fread2(paste0(out_path,"/tmp/PRS_files/PRS_in_all_settings_bychrom/param_chr",allchrom[1],".txt")); nprs <- ncol(score) - 3 # subtract ID columns
  ...
  # Read the full param file (combined from all chromosomes)
  param <- fread2(paste0(out_path, "/tmp/PRS_files/score_param.txt"))
  nprs <- nrow(param)  # This should now match the true number of PRS columns

  # Ensure SCORE and param are properly aligned
  actual_n_scores <- ncol(SCORE)
  expected_n_scores <- nrow(param)

  # Use score column names to find matching parameters (safer if columns are shuffled)
  score_names <- colnames(SCORE)
  param_names <- paste0("score", seq_len(expected_n_scores))

  # Try aligning by name first
  matching_idx <- match(score_names, param_names)

  # Filter out mismatches
  valid_idx <- which(!is.na(matching_idx))
  if (length(valid_idx) < actual_n_scores) {
    cat("Warning: ", actual_n_scores - length(valid_idx), " unmatched score columns removed.\n")
  }

  # Subset to matching parts only
  SCORE <- SCORE[, valid_idx, drop = FALSE]
  param <- param[matching_idx[valid_idx], , drop = FALSE]

  # Final consistency check
  stopifnot(ncol(SCORE) == nrow(param))

  # Filter zero-variance scores
  score_var <- apply(SCORE, 2, var)
  nonzero_var_idx <- which(score_var > 0)

  if (length(nonzero_var_idx) < length(score_var)) {
    cat("Warning: Excluding", length(score_var) - length(nonzero_var_idx), "score(s) with zero variance.\n")
  }

  # Check for valid delta/lambda entries
  valid_param_idx <- which(!is.na(param$delta[nonzero_var_idx]) & !is.na(param$lambda[nonzero_var_idx]))
  final_idx <- nonzero_var_idx[valid_param_idx]

  if (length(final_idx) == 0) {
    stop("No valid parameter combinations (nonzero variance + non-NA delta/lambda).")
  }

  # Compute R²
  R2 <- rep(NA, ncol(SCORE))
  for (i in final_idx) {
  ...
  best <- which.max(R2)
  if (!(best %in% final_idx)) {
    warning("Best R² index (", best, ") is not valid. Choosing next best valid index.")
    best <- final_idx[which.max(R2[final_idx])]
  }

  # Save parameters
  delta0 <- param$delta[best]
  lambda0 <- param$lambda[best]
  cat("Selected best index:", best, "\n")
  cat("delta0:", delta0, "\n")
  cat("lambda0:", lambda0, "\n")
