...
if [[ -n "$APPTAINER_NAME" || -n "$SINGULARITY_NAME" ]]; then
    ...
    SUBMIT_SUF=""
    # conda and the env are already source in the contianer
...
if [ ${rfmix_option} -eq 1 ]; then
  sbatch --wait $SUBMIT_SUF ${path_to_repo}/src/run_rfmix_plots.sh ${WORK} ${REF} ${NAME} ${path_to_repo}
...
############################################ PCA #########################################################
echo "(Step 8) PCA"
sbatch --wait $SUBMIT_SUF ${path_to_repo}/src/run_pca.sh ${WORK} ${REF} ${NAME} ${path_to_repo} 
##########################################################################################################
